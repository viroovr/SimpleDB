...
public class BTPage {
   ...
   private void makeDefaultRecord(BlockId blk, int pos) {
    ...
    // 
    /**
     * Redistributes keys between this page and its sibling.
     * Keys are borrowed from or given to the sibling to achieve a more balanced utilization.
     * @param sibling the sibling page to redistribute keys with
     * @param parent the parent page that holds the reference to this page and the sibling
     * @param direction the direction of redistribution (left or right)
     */
   //  public void redistribute(BTPage sibling, BTPage parent, int direction) {
   //      int myNumRecs = getNumRecs();
   //      int siblingNumRecs = sibling.getNumRecs();

   //      if (direction == -1) {
   //          // Redistribute keys from left sibling to this page
   //          int myNewNumRecs = myNumRecs + 1;
   //          int siblingNewNumRecs = siblingNumRecs - 1;

   //          Constant borrowKey = sibling.getDataVal(siblingNumRecs - 1);
   //          Constant parentKey = parent.getDataVal(findParentSlot(parent, currentblk));

   //          insert(myNumRecs, parentKey, sibling.getChildNum(siblingNumRecs));

   //          for (int i = 0; i < siblingNumRecs - 1; i++) {
   //              Constant val = sibling.getDataVal(i);
   //              int blknum = sibling.getChildNum(i);
   //              insert(myNumRecs + 1 + i, val, blknum);
   //          }

   //          sibling.delete(siblingNumRecs - 1);
   //          parent.updateKey(parentKey, borrowKey);
   //          setNumRecs(myNewNumRecs);
   //          sibling.setNumRecs(siblingNewNumRecs);
   //      } else {
   //          // Redistribute keys from right sibling to this page
   //          int myNewNumRecs = myNumRecs + 1;
   //          int siblingNewNumRecs = siblingNumRecs - 1;

   //          Constant borrowKey = sibling.getDataVal(0);
   //          Constant parentKey = parent.getDataVal(findParentSlot(parent, sibling.currentblk));

   //          insert(myNumRecs, parentKey, sibling.getChildNum(0));

   //          for (int i = 0; i < siblingNumRecs - 1; i++) {
   //              Constant val = sibling.getDataVal(i + 1);
   //              int blknum = sibling.getChildNum(i + 1);
   //              insert(myNumRecs + 1 + i, val, blknum);
   //          }
